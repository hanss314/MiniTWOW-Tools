...
def collect_data(latest):
    ...
    for twow in open('history.txt','r').read().split('\n'):
        ...
        if start_collecting and twow!='':
            to_collect.append('twows/'+twow)

    latest_twow = to_collect[0]

    with open('./{}/results.csv'.format(latest_twow),'r',encoding=encoding) as csvfile:#read responses
        ...
        for row in reader:
            ...
            try:
                votes[name]+=ast.literal_eval(row[7])
            except:
                votes[name] = ast.literal_eval(row[7])

    for twow in to_collect[1:]:
        with open('./{}/results.csv'.format(twow),'r',encoding=encoding) as csvfile:#read responses
            reader = csv.reader(csvfile)
            next(reader, None)
            next(reader, None)
            for row in reader:
    ...
    for twower,vote_list in votes.items():
        mean = statistics.mean(vote_list)
        stdv = statistics.stdev(vote_list)
